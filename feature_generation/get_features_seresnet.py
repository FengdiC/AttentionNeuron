...
att_model = timm.create_model('resnest50d', pretrained=True, features_only=True)
att_model.eval()
...
for img_name in tqdm(os.listdir("dataset/test/")):
    ...
    test_features['resnest50d'][img_name] = {}
    ...
    for stage_idx, stage_features in enumerate(features_att):
        test_features['resnest50d'][img_name][stage_idx] = stage_features.detach().numpy()


with open('test_features_resnest50d.pkl', 'wb') as pkl_f:
    pickle.dump(test_features, pkl_f, protocol=pickle.HIGHEST_PROTOCOL)


train_features = {}
train_features['resnest50d'] = {}
...
for img_name in tqdm(os.listdir("dataset/training/")):
    ...
    # get features only
    features_att = att_model(tensor)
    train_features['resnest50d'][img_name] = {}

    # we got features from 5 different stages in the network
    # dim: [batch, channel, H, W]

    for stage_idx, stage_features in enumerate(features_att):
        train_features['resnest50d'][img_name][stage_idx] = stage_features.detach().numpy()


with open('train_features_resnest50d.pkl', 'wb') as pkl_f:
